...
@router.post("/login")
async def login(
    username: str, password: str, _service: AuthService = Depends(get_auth_service)
) -> TokenResponse:
...
@router.get("/refresh")
async def refresh(
    payload=Depends(require_refresh_token),
    _service: AuthService = Depends(get_auth_service),
) -> TokenResponse:
